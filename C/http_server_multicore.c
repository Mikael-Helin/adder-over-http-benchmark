...
#define NUM_THREADS 6  // Adjust this based on your CPU core count
#define BUFFER_SIZE 256  // Adjust the buffer size as needed

struct thread_arg {
    struct event_base *base;
    struct evhttp *http;
    struct evbuffer *buffer;  // Reusable buffer
};

void request_handler(struct evhttp_request *req, void *arg) {
    struct thread_arg *targ = (struct thread_arg *)arg;
    struct evbuffer *buf = targ->buffer;

    evbuffer_drain(buf, evbuffer_get_length(buf));  // Clear the buffer for reuse
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        thread_args[i].buffer = evbuffer_new();  // Allocate buffer once per thread
        if (!thread_args[i].buffer) {
            puts("Failed to create evbuffer");
            return 1;
        }

        evhttp_set_gencb(https[i], request_handler, &thread_args[i]);
    }
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        evbuffer_free(thread_args[i].buffer);  // Free the buffer when done
    }
